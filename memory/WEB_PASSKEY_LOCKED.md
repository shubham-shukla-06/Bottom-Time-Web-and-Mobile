# WEB PASSKEY FLOW — LOCKED (HEAD updated post-never-ask)
...
3. Post-login enroll prompt: now also persistently suppressed when localStorage.bt_passkey_never_ask is set. The user can opt in/out via the Enroll dialog's "Never ask me again" pill. Flag clears on successful enrollment so the prompt re-arms for future devices.
...
Last verified working on 2026-05-14 (post-never-ask commit).
