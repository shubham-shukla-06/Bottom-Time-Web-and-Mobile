...
export default function Cart() {
  ...
  const handleMoveToWishlist = async (productId) => { try { const res = await axios.post(`/wishlist/product/${productId}`); if (!res.data.wishlisted) await axios.post(`/wishlist/product/${productId}`); await axios.delete(`/cart/${productId}`); toast.success('Moved to wishlist'); refreshCartAndTax(); refreshWishlist(); fetchWishlist(); } catch (e) { toast.error('Failed'); } };
